...
- 起動時ダイアログからの一括削除は `applyAttendance`。結果が 2 つ以上余りうるので、`confirmPendingDeletion` は 1 つ消したあとまだ余っていればピッカーを閉じずに選択値だけ捨てて開き直す（数が揃うまで繰り返す）。ただし余った結果の値がすべて同じ（空欄除外・`trim()` 後の `Set` サイズが 1）なら、どれを消しても結果は変わらないのでピッカーは出さず、先頭から参加者数分だけ残す（空欄の結果欄もこのとき消える）。
...
`edit=1` を付けると、表示直後に `AttendancePickerDialog`（「今回参加する人を選んでください」）を開き、参加する人をチェックボックスで選べる（初期状態は全員未チェック、選んだ人だけが残る）。定例あみだくじ用に配布した URL から当日の参加者だけに絞る用途。ユーザー調査で「欠席者を削除する」より「参加者を選ぶ」方が分かりやすかったため、この形にしている。`p` に有効な参加者が 1 人もいない場合は開かない。
